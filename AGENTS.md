...
- **RFC·제안 Issue**: 논의 중 확정된 단위만 Manyfast 갱신 후 새 구현 Issue(`[feat]`)로 분리해 진행하고, 미확정 안건은 기존 RFC 이슈에 남긴다.
...
- **내용 업데이트 후 바로 버전을 저장하지 않는다.** 에이전트 수정분은 제안 상태로 들어가므로, 사용자가 Manyfast 웹에서 검토·승인(확정)한 것을 확인한 뒤에 버전 저장을 진행한다.
...
**지연 재알림** / **다음 교대 자동 승계** /
**특정 ERP 서식·템플릿 자동 매핑** / **RAG·벡터DB·에이전트 프레임워크** /
**음성 어투·감정 기반 긴급도 판정** / **어르신별 반복 패턴 자동 분석**

> **보호자 전달 "문구 생성"은 범위 안이다.** 범위 밖인 것은 자동 발송과 보호자용 계정·앱이다.
> 출력은 전산 기록 문구와 보호자 전달 문구 두 가지이고, 둘 다 직원이 검토한 뒤 직접 복사한다.

> **파일 출력도 범위 안이다.** 문구를 `.txt` · `.md` · `.xlsx` · `.docx`로 내려받는 것까지는 In이고,
> 특정 ERP의 서식에 맞춘 자동 매핑이 Out이다. 우리는 그 서식을 본 적이 없다.

> **어투·감정 판정 대신 원본 음성 재생을 넣는다.** 어투에서 뽑은 긴급도는 원문에 근거 문장이 없어
> "근거 없는 항목은 만들지 않는다"는 원칙과 충돌한다. 뉘앙스는 원본 음성으로 받는다.

> **어르신 실명은 LLM에 보내지 않는다.** 사전 등록된 명단으로 LLM 호출 전에 룰 기반으로 내부 ID를
> 치환하고, 실명은 화면에 그릴 때만 복원한다. 어르신 등록은 데이터 세팅이지 계정·인증이 아니다.
> **법적 판단은 하지 않는다** — "법적으로 문제없다"가 아니라 "이렇게 설계했다"까지만 말한다.
