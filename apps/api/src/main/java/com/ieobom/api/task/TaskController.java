...
public class TaskController {
	...
	/**
	 * 당일 업무를 미처리와 완료로 나눠 준다. (유저플로우 "AI 인계 도구 내비게이션 맵" n42 관리자 대시보드 · n43 당일 인계·업무 현황)
	 * {@code date} 를 생략하면 오늘이다.
	 *
	 * <p>이 파일의 배정·완료 쪽 번호(n26~n37)는 아직 "새 플로우 3" 기준이다. 두 플로우에서 같은 번호가 다른 화면을 가리키므로 번호와 이름을 함께
	 * 읽는다.
	 *
	...
	/**
	 * 하원 미처리 브리핑. 그날 아직 닫히지 않은 업무만 준다. (유저플로우 n48 브리핑 선택 → n44 하원 미처리 브리핑 · n45 미처리 건수·목록)
	 *
