...
export function AdminStaffPinResetSection() {
  ...
  return (
    <section aria-labelledby="pin-reset-heading" className="mt-8 rounded-2xl border border-border-card bg-surface-card p-6">
      <h2 id="pin-reset-heading" className="text-2xl font-bold text-ink">
        직원 PIN 초기화 관리
      </h2>
      ...
      {feedbackMessage && (
        <div
          role="status"
          className="mt-4 rounded-xl bg-success/10 p-3 text-base font-semibold text-success"
        >
          ✓ {feedbackMessage}
        </div>
      )}
      ...
      {directory.isPending ? (
        <p className="mt-4 text-lg text-ink-muted">직원 명단을 불러오는 중입니다…</p>
      ) : directory.isError ? (
        <p className="mt-4 text-lg text-primary">직원 명단을 불러오지 못했습니다.</p>
      ) : staffList.length === 0 ? (
        <p className="mt-4 text-lg text-ink-muted">등록된 직원이 없습니다.</p>
      ) : (
        <ul className="mt-4 divide-y divide-border-divider">
          {staffList.map((staff) => (
            <li key={staff.code} className="flex flex-wrap items-center justify-between gap-3 py-3">
              <div className="flex items-center gap-3">
                <span className="text-lg font-bold text-ink">{staff.name}</span>
                {staff.jobRoleLabel && (
                  <span className="rounded-lg bg-primary-soft px-2 py-0.5 text-sm font-semibold text-primary">
                    {staff.jobRoleLabel}
                  </span>
                )}
                <span className="text-base text-ink-muted">{staff.code}</span>
                {staff.hasPin ? (
                  <span className="rounded-lg bg-amber-50 px-2 py-0.5 text-xs font-semibold text-amber-700">
                    🔒 PIN 설정됨
                  </span>
                ) : (
                  <span className="rounded-lg bg-btn-neutral px-2 py-0.5 text-xs font-semibold text-ink">
                    무인증
                  </span>
                )}
              </div>
