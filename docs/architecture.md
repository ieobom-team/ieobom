...
├── shared/ui/             여러 화면이 함께 쓰는 UI 조각 (`BigButton`, `AppHeader`, `BottomNav`, `PageLayout`)
...
**공통 레이아웃 및 네비게이션 표준 (`shared/ui/PageLayout`)**:
- **상단 통합 헤더 (`AppHeader`)**: 모든 내부 화면에서 `[뒤로가기 / 로고(홈)] [화면 제목] [본인 칩 / 본인 바꾸기]`를 제공하여 페이지별 산발적인 이동 링크를 통일한다.
- **현장 모바일 바텀 네비게이션 (`BottomNav`)**: 현장 근무자 주요 뷰(`/field` 홈, `/handover-cards` 인계 카드, `/tasks` 오늘의 업무)의 모바일 화면에서 엄지손가락으로 즉시 전환 가능한 3탭 바텀 바를 제공한다.
- **입력/상세 폼**: 폼 작성 및 수정 화면에서는 오터치 방지를 위해 바텀 탭을 숨기고, 최하단 고정 액션 버튼(Sticky Action Button)으로 조작을 완결한다.
...
본인 식별은 **직원 이름 또는 사번 선택**으로 하며 기본적으로 비밀번호를 강제하지 않는다.
단, 공용 기기 오용 방지 및 개인 알림 보호를 위해 **선택형 4~6자리 숫자 PIN**을 지원한다. (`F-YJJJUX` permissions, [#83](https://github.com/ieobom-team/ieobom/issues/83), [#84](https://github.com/ieobom-team/ieobom/issues/84))
...
- 기기 하나를 여러 직원이 돌려 쓰므로 홈 헤더(`SessionHeader`)에 **본인 바꾸기**와 **PIN 설정/변경**을 둔다.
- **선택형 PIN 검증 & 관리 UI**:
  - `PinPadModal.tsx`: PIN이 설정된 직원 진입 시 0~9 터치 키패드 및 물리 키보드로 4~6자리 PIN 검증. (5회 연속 실패 시 1분간 `423 Locked` 잠금)
  - `PinSettingsModal.tsx`: 직원이 4~6자리 숫자 PIN을 신규 등록, 변경, 해제하는 모달.
  - `AdminStaffPinResetSection.tsx`: 관리자 화면에서 직원의 PIN을 원클릭으로 초기화(무인증 복구)하는 컴포넌트.
