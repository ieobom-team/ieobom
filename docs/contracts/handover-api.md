...
입력 화면이 **대상 어르신을 고르기 위해** 부른다.
`F-YJJJUX` preconditions("입력할 어르신이 목록에 존재해야 한다")를 채우는 조회다.

한 센터 20명 남짓이라 페이지를 나누지 않고 전원을 내려준다.
...
| `name` | string | 이름 |
| `code` | string | 센터 내 식별번호. 동명이인을 화면에서 구분한다 |

**이름 가나다순이고, 이름이 같으면 식별번호순이다.** id 순이 아니다.
