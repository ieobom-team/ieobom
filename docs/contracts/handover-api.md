...
> **모든 기기가 `audioData` 를 보낸다.** (#147) 인식이 서버로 옮겨 가면서
> (`POST /api/handovers/transcribe`) 기기는 녹음만 하면 되고, 마이크 경합 때문에 모바일에서
> 원본 음성을 포기했던 #146 의 제약이 사라졌다. 계약 자체는 처음부터 달라진 것이 없다 —
> `audioData` 는 여전히 선택 필드다.
>
> 화면은 **같은 음성을 두 번 올린다.** 한 번은 글로 바꾸려고(`/transcribe`), 한 번은 저장하려고
> 여기로. 변환 시점에는 인계 기록이 아직 없고 직원이 글을 고칠 기회가 먼저 와야 하기 때문이다.
...
## `POST /api/handovers/transcribe`

녹음한 음성을 글로 바꾼다. (Manyfast `F-YJJJUX` rules — 기기가 녹음만 하고 서버가 그 음성을 글로 바꿔 돌려준다)

**인계 id 를 받지 않고, 아무것도 저장하지 않는다.** 직원이 마이크를 멈춘 직후 — 글을 확인하고
어르신을 고르기 전 — 에 부르기 때문이다. 저장은 확인이 끝난 뒤 `POST /api/handovers` 가 한다.

브라우저 내장 음성 인식은 쓰지 않는다. 모바일은 페이지가 마이크를 잡는 순간 내장 인식이 굶어
녹음과 인식을 함께 할 수 없고(#146), 기기마다 결과를 쌓는 방식도 달라 같은 말이 겹쳐 쌓였다(#149 · #151).
그래서 **모든 기기가 같은 경로를 쓴다** — 기기별 분기를 두지 않는다.

### 요청

| 필드 | 타입 | 필수 | 설명 |
|---|---|---|---|
| `audioData` | string | ✅ | 녹음한 원본 음성. Base64 Data URL(`data:audio/…;base64,…`) |

`POST /api/handovers` 의 `audioData` 와 **같은 형식·같은 상한**이다. 화면이 이미 그 형식으로 녹음을
들고 있고, 서버도 같은 파서와 같은 10MB 검증을 그대로 쓴다. 갈라 두면 변환은 통과했는데 저장에서
막히는 음성이 생기고, 그때는 이미 마이크를 놓은 뒤라 직원이 되돌릴 방법이 없다.

```json
{
  "audioData": "data:audio/webm;codecs=opus;base64,GkXf..."
}
```
...
**다듬지 않은 인식 결과 그대로다.** 서버가 원문을 손대지 않는다 — 틀린 곳은 직원이 화면에서 고친다.
아무 말도 담기지 않았으면 `text` 는 빈 문자열이며, 이것은 오류가 아니다.

### 오류

| 상황 | 결과 |
|---|---|
| `audioData` 없음 | `400` — `fields[].field = "audioData"` |
| Data URL 이 아니거나 형식이 `audio/*` 가 아님 | `400` — `fields[].field = "audioData"` |
| 10MB 초과 | `400` — `fields[].field = "audioData"` |
| `LLM_API_KEY` 미설정 · 제공자 거부 · 응답을 읽지 못함 | `503` — `code: "LLM_UNAVAILABLE"` |

**`503` 은 입력을 잃는 실패가 아니다.** 화면은 이때도 녹음한 원본 음성을 들고 있고, 글 칸에 직접
입력해 저장을 마칠 수 있다. (Manyfast `F-YJJJUX` rules — "변환이 실패해도 녹음된 원본 음성과 직접
입력할 수 있는 글 칸은 남아 있다") 연결이 끊겨 요청 자체가 실패한 경우도 같게 다룬다.

### 어르신 실명

음성 원본에는 실명이 그대로 들어 있고, **문자열 대조로 하는 내부 ID 치환을 오디오에는 적용할 수 없다.**
그래서 실명이 담긴 채로 외부 인식 서비스에 전달된다. 브라우저 내장 인식도 오디오를 브라우저
제조사의 서버로 보냈으므로 서버 인식을 쓰기 전부터 그러했다.

**치환 순서는 바뀌지 않는다** — 음성이 글이 된 뒤, LLM 을 부르기 전에 룰 기반으로 적용한다.
(Manyfast `F-LUDCWW` rules) 어르신 명단을 인식 정확도를 높이는 힌트로 함께 보내지 않는다.

### 모델

`ieobom.llm.transcription-model`(env `LLM_TRANSCRIPTION_MODEL`, 기본 `gpt-4o-transcribe`)이 정한다.
`base-url` 과 마찬가지로 설정값이라 제공자·모델 교체에 코드 변경이 필요 없다.
...
**재생 단위는 입력 한 건 전체다.** 항목별 구간 재생은 인식 응답에서 구간 정보를 함께 받아야
하는데, 지금은 글만 받는다. (Manyfast `F-SNBVHR` dataSpec 미결 질문 — #44 · #147 범위 제외)
...
- 연결이 끊긴 상태에서는 변환도 할 수 없다. 그 경우는 **변환 실패와 같게** 다뤄 직원이 글 칸에 직접
  입력하게 하고, 저장은 위 대기열 경로를 그대로 탄다. (#147)
