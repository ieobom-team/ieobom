...
배정 화면은 빈 입력이 아니라 **AI 제안값이 채워진 상태**로 열린다. (`F-IVFNPC` display, 유저플로우 "새 플로우 3" n28)
그 값은 카드가 이미 들고 있다.
...
유저플로우 "새 플로우 3"에서도 카드 상세(n21)는 검토(n25)와 배정(n26) 두 갈래로 함께 열려 있다. 무엇보다 **문구는
...
그날 업무 목록이다. (유저플로우 "새 플로우 3" n31 · n32 / "AI 인계 도구 내비게이션 맵" n42 · n43) `date` 를 생략하면 오늘이다.
...
```json
{
  "date": "2026-08-12",
  "tasks": [
    { "id": 4, "status": "PENDING", "dueTime": "09:00", "...": "위 `201` 과 같은 모양" },
    { "id": 7, "status": "PENDING", "dueTime": "17:30", "...": "..." },
    { "id": 3, "status": "DONE", "dueTime": "10:00", "...": "..." }
  ],
  "pending": [{ "id": 4, "...": "..." }, { "id": 7, "...": "..." }],
  "done": [{ "id": 3, "...": "..." }],
  "pendingCount": 2,
...
* `tasks`: 전체 업무 목록 (현장 근무자 목록 화면 `TaskListPage`용, 미처리 우선 정렬)
* `pending` / `done`: 상태별 분리 목록 (관리자 대시보드 `OperationsDashboardPage`용)
* `pendingCount` / `doneCount`: 당일 미처리 및 완료 건수

**미처리를 먼저, 그 안에서는 기한이 이른 순으로 준다. 완료는 뒤에 두고 같은 기준으로 정렬한다.**
Manyfast에 없는 결정이라 구현에서 정했다 — 화면이 먼저 봐야 하는 건 아직 안 닫힌 일이고, 그중에서도
곧 닫혀야 하는 일이다.

**어르신별로 묶지 않는다.** 인계 카드 목록과 다른 점이다. 업무는 이미 담당자·기한이 붙어 있어
"누구 일인지"보다 "누가 언제까지 무엇을 해야 하는지"가 먼저다.

**하루는 업무가 만들어진 시각(`createdAt`)으로 가른다.** 카드가 만들어진 날이 아니다. 기한이 항상
당일 시각이므로, 업무를 배정한 날이 곧 그 업무가 속한 날이다.

---

## `GET /api/tasks/{taskId}`

업무 하나. (유저플로우 "새 플로우 3" n34) 응답은 위 `201` 과 같은 모양이다.
